...
class RunningShootingGuy(pygame.sprite.Sprite):
    def __init__(self, start, end, y, points):
        ...
        self.place_x = end
        self.move = 'left'
        self.image = pygame.transform.scale(pygame.transform.flip(self.frames_run[self.i], 90, 0),
                                            (300, 300))
        self.rect = self.image.get_rect()
        self.mask = pygame.mask.from_surface(self.image)
        self.rect.y = y
                ...
                if self.i >= len(self.frames_attack):
                    ...
                    if self.move == 'left':
                        self.image = pygame.transform.scale(
                            pygame.transform.flip(self.frames_attack[self.i], 90, 0), (1200, 300))
                    else:
                        self.image = pygame.transform.scale(self.frames_attack[self.i], (1200, 300))
                del self.were[-1]
                self.were.append(tm)
        if not self.shoot:
            self.i += 1
    ...
    def update(self):
        ...
        if run_away:
            ...
            self.image = pygame.Surface((self.size_x, self.size_y),
                                        pygame.SRCALPHA, 32)
            pygame.draw.rect(self.image, pygame.Color("black"), (0, 0, self.size_x, self.size_y))
...
portal = AnimatedSprite(load_image("Purple Portal Sprite Sheet.png"), 8, 1, 800, 350, (300, 300))
...
drone_portal1 = AnimatedSprite(load_image('Guardian Sprite-Sheet.png'), 1, 39, 3600, 280,
                               (350, 200), True)
shooter = AnimatedSprite(pygame.transform.flip(load_image('blood explosion gun spell-Sheet.png'),
        ...
        if event.type == pygame.USEREVENT:
            if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element in (
                        button_back_main_window, button_last_save, button_back_main_window_pause,

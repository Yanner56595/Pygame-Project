...
class Star:
    def __init__(self):
        ...
        self.color = choice([(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0), (0, 255, 255),
                             (255, 0, 255)])
...
def start_window(width, height, LANGUAGE):
    play = 0
    try:
...
def finish_window(width, height, distance_m, name, sunduk, LANGUAGE):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if 50 <= event.pos[0] <= 350 and 550 <= event.pos[1] <= 730:
                    button_finish.play()
                    return main_window(width, height, LANGUAGE)
...
def main_window(width, height, LANGUAGE):
    sunduk = 0
    update(sunduk)
    BOY = [load_image("boy1.png"), load_image("boy5.png"), load_image("boy2.png"),
           load_image("boy3.png"), load_image("boy4.png")]
    BOY_NIKE = [load_image("boynike1.png"), load_image("boynike5.png"),
                load_image("boynike2.png"), load_image("boynike3.png"), load_image("boynike4.png")]
    ...
    CHOOSE = ["stone", "npc", "boots", "bag", "boots", "npc", "stone", "stone", "stone", "stone",
              "stone", "sunduk"]
    y = 275
    k1 = 0
    boy_bag = 0
    ...
    while running:
        try:
                    ...
                    for i in range(len(st_list)):
                        ...
                        if st_list[i][0] == "npc":
                            ...
                            for j in range(len(balls)):
                                ...
                                m = [x5, y5, w5, h5]
                                if not(n[0] + n[2] < m[0] or m[0] + m[2] < n[0] or
                                       n[1] + n[3] < m[1] or m[1] + m[3] < n[1]):
                                    st_list[i][0] = "fair"
            ...
            exception += 1
    return finish_window(width, height, distance_m, name, sunduk, LANGUAGE)


def level2(LANGUAGE):
    path(transfer, k5)
    start_window(width, height, LANGUAGE)
    my_result = main_window(width, height, LANGUAGE)
    return my_result
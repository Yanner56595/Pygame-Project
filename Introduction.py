...
def introduction_level(LANGUAGE, size, width, height):
    global show_titles, x, outside, inside, all_sprites, hero, y
        ...
        def update(self, where, move=True):
            ...
            if move:
                ...
                if outside:
                    ...
                    if where == 'right':
                        self.image = walk_right_big[self.i]
                        self.rect.x += 4
                    else:
                        self.image = walk_left_big[self.i]
                        self.rect.x -= 4
            else:
                if outside:
                    snow.stop()
                ...
                if outside:
                    ...
                    if where == 'right':
                        self.image = stay_right_big
                    else:
                        self.image = stay_left_big
            self.mask = pygame.mask.from_surface(self.image)
            if outside:
                x_new = max(-150, x_new)
                if x_new >= 930:
            ...
            if (outside and 300 <= x_new <= 400 and not first_titles_shown) or \
                    ...
                    (
                            inside and self.second_room and 100 <= x_new <= 200 and
                            not kitchen_titles_shown) \
    ...
    walk_left = [load_image('L1_mainhero.png'), load_image('L2_mainhero.png'),
    ...
    walk_right = [pygame.transform.flip(load_image('L1_mainhero.png'), 90, 0),
    ...
    stay_left = load_image('L5_mainhero.png')
    stay_right = pygame.transform.flip(load_image('L5_mainhero.png'), 90, 0)

    walk_left_big = [load_image('L1_mainherobig.png'), load_image('L2_mainherobig.png'),
    ...
    walk_right_big = [pygame.transform.flip(load_image('L1_mainherobig.png'), 90, 0),
    ...
    stay_left_big = load_image('L5_mainherobig.png')
    stay_right_big = pygame.transform.flip(load_image('L5_mainherobig.png'), 90, 0)
    ...
    death = pygame.transform.flip(pygame.transform.scale(load_image('Death.png'), (500, 650)), 90,
                                  0)
    ...
    portal_breaks_blue = AnimatedSprite(
        pygame.transform.scale(load_image('Mirror portal breaks.png'),
    ...
    while running:
        if freddy_scene:
            ...
            achievements.append('Смерть от Фредди')
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.USEREVENT:
                    if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                        if event.ui_element in (button_back_main_window,
                                button_back_main_window_pause):
                            if not deleted_files:
                                for name in names_freddy:
                                    os.remove(os.path.join(directory, name))
                            return 0, achievements
                        if event.ui_element in (button_last_save_pause, button_last_save):
                            if not deleted_files:
                                for name in names_freddy:
                                    os.remove(os.path.join(directory, name))
                ...
                if not show_titles:
                    ...
                    if keys[pygame.K_LEFT]:
                        ...
                        if result == 1:
                            dead = True
                            get_achievement.play()
                            achievements.append('Смерть от тортика')
                        if result == 2:
                            freddy_scene = True
                    else:
                    ...
                    if outside:
                        ...
                        if 300 <= x <= 400 and not first_titles_shown:
                            if SOUND_TITLES:
                                first_titles_shown, pressed_enter = first_titles.update(screen,
                                                                                        clock,
                                                                                        pressed_enter,
                                                                                        'black',
                                                                                        typing,
                        ...
                        if not hero.second_room and 0 <= x <= 250 and lobby_titles_shown:
                            ...
                            if hero.second_room and 100 <= x <= 200 and kitchen_titles_shown and \
                                ...
                                if kitchen_titles_second_shown:
                                    show_titles = False
                                    pressed_enter = False
                            if not hero.second_room and kitchen_titles_second_shown and \
                                    800 <= x <= 900 and not lobby_titles_shown:
                                if SOUND_TITLES:
                                    lobby_titles_shown, pressed_enter = lobby_titles.update(
                                        screen,
                                        clock, pressed_enter, 'black', typing, end_typing)
                                else:
                                    lobby_titles_shown, pressed_enter = lobby_titles.update(
                                        screen,
                                        clock,

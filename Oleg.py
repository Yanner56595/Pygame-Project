...
class Unit(AnimatedSprite):
    def __init__(self, unit_type, tex_type, tex_holder, columns, frame_time, tbl, x, y,
        ...
        AnimatedSprite.__init__(self, tex_holder.get(tex_type), columns, frame_time, tbl, x, y,
                                board.units)
    ...
    def attack(self, tar, show_effects=True):
        ...
        if tar.current_health <= 0:
            ...
            if show_effects:
                tar.hp_bar.hide()
                create_particle(tar.tar_coords, self.blood_set, -2, 2, -2, 2, 1, 2, 20,
                                self.board.particles)
    ...
    def update(self, time_delta):
        ...
        if self.rec_dmg:
            ...
            if self.wound_cur < 0:
                self.wound_cur = self.wound_time
                self.rec_dmg = 0
                create_particle(self.rect.center, self.blood_set, -2, 2, -2, 2, 1, 2, 10,
                                self.board.particles)

        if self.rect.x != self.tar_coords[0]:
            self.rect.x = round(move_towards(self.rect.x, self.tar_coords[0],
                                             self.moving_speed * time_delta))

        elif self.rect.y != self.tar_coords[1]:
            self.rect.y = round(move_towards(self.rect.y, self.tar_coords[1],
                                             self.moving_speed * time_delta))
...
class Intro:
    def __init__(self, tex_holder, snd_holder, lang="ru"):
        ...
        self.satan_img = pygame.transform.scale(self.satan_img, (
            int(self.satan_img.get_width() * 0.7), int(self.satan_img.get_height() * 0.7)))
        ...
        self.scrooge_img = pygame.transform.scale(self.scrooge_img, (
            int(self.scrooge_img.get_width() * 0.7), int(self.scrooge_img.get_height() * 0.7)))
...
class OverScreen:
    def __init__(self, manager):
        ...
        self.new_game = pygame_gui.elements.UIButton(relative_rect=new_game_rect,
                                                     container=self.panel,
                                                     manager=manager, text="lang.new_game")
        self.exit_game = pygame_gui.elements.UIButton(relative_rect=new_game_rect.move(0, 40),
                                                      container=self.panel,
                                                      manager=manager, text="lang.exit")
        self.game_over = pygame_gui.elements.UILabel(relative_rect=new_game_rect.move(0, -200),
                                                     container=self.panel,
...
class VictoryScreen:
    def __init__(self, manager, tex_holder, user_id):
        ...
        self.panel = pygame_gui.elements.UIPanel(relative_rect=panel_rect, starting_layer_height=0,
                                                 manager=manager)
        new_game_rect = pygame.Rect((200, 300), (200, 40))
        self.next_level = pygame_gui.elements.UIButton(relative_rect=new_game_rect.move(0, -40),
                                                       container=self.panel,
        ...
        self.exit_game = pygame_gui.elements.UIButton(relative_rect=new_game_rect.move(0, 40),
                                                      container=self.panel,
                                                      manager=manager, text="lang.exit")
        self.victory = pygame_gui.elements.UILabel(relative_rect=new_game_rect.move(0, -200),
                                                   container=self.panel,
...
class GameUI:
    def __init__(self, manager):
        ...
        self.end_turn_btn = pygame_gui.elements.UIButton(relative_rect=end_turn_btn_rect,
                                                         text='lang.finish',  manager=manager)
            ...
            if unit.turn == turn:
                ...
                if is_max:
                    ...
                    for i in range(len(steps_stack)):
                        ...
                        best_weight = max(best_weight, self.minimax(deph - 1,
                    ...
                    for i in range(len(steps_stack)):
                        new_move = steps_stack.pop()
                        self.do_step(new_move)
                        best_weight = min(best_weight, self.minimax(deph - 1,
                                                                    not is_max, alpha, beta))
                        self.undo_step(new_move)
...
class GameManager:
    ...
    def load_sounds(self):
        self.snd_holder.load(Sounds.TYPING, "typing1.mp3")
    ...
    def on_ui_button_pressed(self, event):
        if event.ui_element == self.game_ui.end_turn_btn and self.is_game:
            self.end_step()
        if event.ui_element in (self.victory_screen.exit_game, self.victory_screen.next_level):
            self.won = True
    ...
    def start_game(self):
        ...
        if self.won and self.next:
            return 3
        if self.won and not self.next:
            return 2
        return 0
...
class Board:
    ...
    def render(self, target):
        ...
        buffer = pygame.transform.scale(buffer, (
            buffer.get_width() * self.scale, buffer.get_height() * self.scale))
    ...
    def get_cell(self, mouse_pos):
        ...
        if (0 <= column < self.width and 0 <= row < self.height and
                0 <= mouse_x - column * (
                        self.cell_size + 2 * self.outline_width
                ) - self.outline_width <= self.cell_size and 0 <= mouse_y - row * (
                        self.cell_size + 2 * self.outline_width
                ) - self.outline_width <= self.cell_size):
...
def level1(LANGUAGE, user_id):
    pygame.init()
    if LANGUAGE == 'Русский':
        game_manager = GameManager("map1.csv", user_id, lang="ru")
    else:
        game_manager = GameManager("map1.csv", user_id, lang="en")
